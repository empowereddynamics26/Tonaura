...
function planKeyFromSession(obj) {
  const fromMeta = obj.metadata?.price_key;
  if (fromMeta === "monthly" || fromMeta === "yearly" || fromMeta === "lifetime") return fromMeta;
  const amount = obj.amount_total;
  if (amount === 3999 || obj.mode === "payment") return "lifetime";
  if (amount === 2499) return "yearly";
  if (amount === 399) return "monthly";
  return obj.mode === "payment" ? "lifetime" : "monthly";
}
...
async function resolveUserId(admin, { userId, customerId, email }) {
  ...
  if (email) {
    try {
      ...
      // Auth admin lookup is best-effort; checkout should send client_reference_id.
    }
  }
  return null;
}
...
export async function applyStripeEvent(event) {
  ...
  if (type === "checkout.session.completed") {
    ...
    const planKey = planKeyFromSession(obj);
    const lifetime = planKey === "lifetime" || obj.mode === "payment";
    await upsertEntitlement(admin, resolved, {
      is_active: true,
      plan_key: lifetime ? "lifetime" : planKey,
      product_id: lifetime ? "lifetime" : planKey || null,
    ...
    const payEmail = obj.customer_details?.email || obj.customer_email;
    if (payEmail) {
      await Promise.allSettled([
        sendPaymentConfirmedEmail({
          to: payEmail,
          planLabel: planLabelFromKey(lifetime ? "lifetime" : planKey),
        }),
      ]);
    }
    return { ok: true };
  ...
  if (
    ...
    if (type === "customer.subscription.deleted") {
      let subEmail = null;
      try {
        const { data: authUser } = await admin.auth.admin.getUserById(resolved);
        subEmail = authUser?.user?.email || null;
      } catch {
        /* ignore */
      }
      if (subEmail) {
        await Promise.allSettled([
          sendSubscriptionEmail({
            to: subEmail,
            planLabel: planLabelFromKey(planKey),
            active: false,
          }),
        ]);
      }
  ...
  if (type === "invoice.paid" || type === "invoice.payment_failed") {
    ...
    if (type === "invoice.payment_failed") {
      ...
      let failEmail = obj.customer_email || null;
      if (!failEmail) {
        try {
          const { data: authUser } = await admin.auth.admin.getUserById(resolved);
          failEmail = authUser?.user?.email || null;
        } catch {
          /* ignore */
        }
      }
      if (failEmail) {
        const planKey =
          obj.lines?.data?.[0]?.price?.recurring?.interval === "year"
            ? "yearly"
            : obj.lines?.data?.[0]?.price?.recurring?.interval === "month"
              ? "monthly"
              : "monthly";
        await Promise.allSettled([
          sendPaymentFailedEmail({
