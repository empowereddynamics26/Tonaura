...
function siteUrl() {
  return env("NEXT_PUBLIC_SITE_URL", "https://tonaura.io").replace(/\/$/, "");
}
...
function wrap({ eyebrow = "Tonaura", title, bodyHtml, footerNote }) {
  ...
  const note =
    footerNote ||
    `Tonaura · Solfeggio tone therapy<br/><a href="${site}" style="color:${GOLD};text-decoration:none;">tonaura.io</a> · <a href="mailto:support@tonaura.io" style="color:rgba(237,231,217,0.55);text-decoration:none;">support@tonaura.io</a>`;
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8"/>
